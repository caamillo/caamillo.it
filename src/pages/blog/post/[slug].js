...
export default function Post({ post }) {
    ...
    return (
        <BlogLayout>
            <div ref={ nsfwModalRef } data-show="false" className="nsfw-warning w-screen h-screen fixed top-0 left-0 duration-500 z-[999999] transition-all">
                <div className='absolute top-0 left-1/2 -translate-x-1/2 container flex justify-center'>
                    <div className='warning-modal transition-transform duration-1000 ease-in-out bg-white w-full lg:w-2/3 xl:w-1/2 p-5 rounded-lg mt-10 mx-3 border-2 border-slate-300 flex flex-col items-center text-center'>
                        ...
                        <div className='grid md:grid-cols-2 w-full mt-5 gap-3 md:gap-5'>
                            <Link href='/blog' className='outline-2 outline-slate-300 text-slate-700 outline rounded-lg py-4 order-1 font-semibold'>Go Back</Link>
                            <button onClick={ () => setNsfw(true) } className='bg-slate-800 text-white rounded-lg py-4 md:order-2 font-semibold'>Sure.</button>
                        </div>
                    </div>
                </div>
            </div>
