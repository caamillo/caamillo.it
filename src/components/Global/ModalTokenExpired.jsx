        ...
        <Modal modalRef={ tokenExpiredRef }>
            ...
            <div className='grid md:grid-cols-2 w-full mt-5 gap-3 md:gap-5'>
                <Link onClick={() => {
                    ...
                    }} href='/login' className='border-2 border-slate-300 text-slate-900  rounded-lg py-4 order-1 font-semibold'>Go back to Login</Link>
                <button onClick={() => {
                    setToken(undefined)
                    setIsTokenValid(true)
                    }} className='bg-slate-900 text-white border-2 border-slate-900 rounded-lg py-4 md:order-2 font-semibold'>Sure.</button>
            </div>
        </Modal>
    )
}